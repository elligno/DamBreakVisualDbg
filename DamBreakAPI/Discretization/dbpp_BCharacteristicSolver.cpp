...
namespace dbpp {
double BCharacteristicSolver::computeDownstreamValues(
    const Nodal_Value
  ...
  const auto V1 =
      std::get<1>(w_nval) /
      std::get<0>(w_nval); // U2[NbSections - 2] / U1[NbSections - 2];
  const auto R1 = HydroUtils::R(
      std::get<0>(w_nval),
      aLastSection
          .B()); // HydroUtils::R(U1[NbSections - 2], 1./*w_sectDown->B()*/);
  const auto Y1 =
      std::get<2>(w_nval) -
      aLastSection
          .Z(); // H[NbSections - 2] - 0./*w_sectDown->Z()*/; //Z[NbSections-2];
  //       const double CA =
  ...
  const auto CP =
      V1 + CA * Y1 +
      PhysicalConstant::sGravity * (aLastSection.getS0am() - Sf1) * aDt;
...
double BCharacteristicSolver::computeUpstreamValues(
  ...
  const auto V0 = CN + CB * (aSectFlow0.H() -
                             aSectFlow0.Z()); // CB*(H[0] - Z[0]);  velocity at
                                              // upstream along characteristic
