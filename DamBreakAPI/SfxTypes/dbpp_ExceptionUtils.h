...
public:
  ...
  explicit Error(const std::string &what = "") : m_Message(what) {
    // nothing to do
  }
...
public:
  // ctor
  explicit AssertionFailedError(const std::string &what = "") : Error(what) {}
};
...
public:
  // ctor
  explicit IllegalArgumentError(const std::string &what = "") : Error(what) {}
};
