...
//   cellface_0 = pair of cell face (0,1)/(1,2) because the first node is a tied
node
//   for now we assume this discretization. Create a cell which is made of a
pair
//   of cell face.
//
//   Design Note:
...
*/
class cellFace
    : public boost::equality_comparable<cellFace>, // strickly comparable
...
public:
  ...
  typedef std::pair<unsigned /*left node index*/, unsigned /*right node index*/>
      pairnodeI;
...
public:
  ...
  cellFace(
      unsigned short aCellFaceIdx, // global index (global discretization)
      unsigned short aLeftNodeIdx,
      unsigned short aRightNodeIdx, // grid node index left/right on both side
      /*double aLeftx, double aRightx,*/ bool aLneighbour =
  ...
  friend bool operator<(const cellFace &lhs, const cellFace &rhs) {
    return lhs.m_cellFaceI < rhs.m_cellFaceI &&
           lhs.m_leftNodeI < rhs.m_leftNodeI;
  }
...
// 		{
// 			return aI==0 ? std::get<0>(m_rightVec) :
// std::get<1>(m_rightVec);
// 		}
// read-only
