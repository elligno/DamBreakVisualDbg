...
{
	/** 
	 * Brief Implementation that use the scalar field to implement the predictor/corrector algorithm.
	 *   Numerical scheme representation that inherit from base class 
	 *	 We still use the U1/U2 variables of the base
	 *   class, because we still using deprecated algorithm
	 *   which we need to pass as argument std::vector.
	 *   BaseNumTreatment is deprecated.
	 *
	 *   Design Note inherit from boost noncopyable to denied access
	 *   of copying and assignment. Also is final, cannot be inherited or sub-classed.
	 */
